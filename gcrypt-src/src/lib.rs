...
pub fn source_dir() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("gcrypt")
}
...
impl Build {
    pub fn new() -> Self {
        Self {
            out_dir: env::var_os("OUT_DIR").map(|s| PathBuf::from(s).join("gcrypt-build")),
        }
    }
    ...
    pub fn build(&mut self) -> Artifacts {
        let gpgrt_artifact = gpgrt_src::Build::new().build();
